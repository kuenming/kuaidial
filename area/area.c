...
Area *Area_load(const char *filename, unsigned int cache_size){
	...
	if(area->info->version != AREA_VERSION || area->info->id != AREA_ID ||
		!area->info->prefix_index.offset || !area->info->prefix_index.size ||
		!area->info->suffix_index.offset || !area->info->suffix_index.size ||
		!area->info->node_data.offset || !area->info->node_data.size ||
		!area->info->name_data.offset || !area->info->name_data.size ||
		area->info->max_lenght <= 0 || area->info->min_lenght <= 0){
		Area_unload(area);
		return NULL;
	}
	...
	area->buffer = (char *)malloc(AREA_MAX_NAME_LENGHT * 2 + AREA_MAX_INFO_LENGHT);
	if(!area->buffer){
		Area_unload(area);
		return NULL;
	}
			...
			for(;;){
				...
				if(find_suffix >= find.suffix_start && find_suffix <= find.suffix_end &&
					...
					if(node.type_offset && (withs & AREA_WITH_TYPE)){
						...
						if(area->cache){
							unsigned int len = strlen(area->cache + node.type_offset);
